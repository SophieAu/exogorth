...
public class Player {
	...
	public Player(int geschwindigkeit, int startX, int startY, int WeltX, int WeltY, ArrayList<Bullets> bullet)
		...
		playerPicture[0] = ImageIO.read(getClass().getClassLoader().getResourceAsStream("bilddateien/LK21.gif"));
		playerPicture[1] = ImageIO.read(getClass().getClassLoader().getResourceAsStream("bilddateien/LK21.gif"));
		playerPicture[2] = ImageIO.read(getClass().getClassLoader().getResourceAsStream("bilddateien/LK21.gif"));
	...
	public void update() throws IOException {
		...
		if (playerX > ((worldX * 2) / 3))
			...
			endOfWorld = false;
		// Regenbogen
		bullet.add(new Bullets(-speed, playerY + (playerPicture[timeCounterForAnimation].getHeight() / 2) - 9, playerX,
				1, bullet));
		// Bewegungsreaktion
		if (zeitani < 1) {
			zeitani = 100;
